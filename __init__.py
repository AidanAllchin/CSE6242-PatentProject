...
def os_specific_download(url: str, dest: str):
    ...
    # mac doesn't have wget
        ...
        print(f"{Fore.RED}[init]: {Style.NORMAL}Please download the data manually from the USPTO website:\n\t{url}{Style.RESET_ALL}")
        sys.exit(1)
...
def os_specific_unzip(file: str, dest: str):
    ...
    # mac doesn't have wget
        ...
        print(f"{Fore.RED}[init]: {Style.NORMAL}Unsupported operating system: {sys_type}{Style.RESET_ALL}")
        print(f"{Fore.RED}[init]: {Style.NORMAL}Please download the data manually from the USPTO website: {file}{Style.RESET_ALL}")
        sys.exit(1)
...
def download_raw_tables():
    ...
    if not os.path.exists(patents_g_p):
        print(f"\n{Fore.YELLOW}[init]: Downloading {Style.DIM}g_patent.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_download("https://s3.amazonaws.com/data.patentsview.org/download/g_patent.tsv.zip", zip_instead(patents_g_p))
        
        # unzip
        print(f"{Fore.LIGHTMAGENTA_EX}[init]: Unzipping {Style.DIM}g_patent.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_unzip(zip_instead(patents_g_p), os.path.dirname(patents_g_p))
    else:
        print(f"{Fore.GREEN}[init]: {Style.DIM}g_patent.tsv{Style.NORMAL} already exists.{Style.RESET_ALL}")

    if not os.path.exists(applicant_g_p):
        print(f"\n{Fore.YELLOW}[init]: Downloading {Style.DIM}g_inventor_not_disambiguated.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        ...
        # unzip
        print(f"{Fore.LIGHTMAGENTA_EX}[init]: Unzipping {Style.DIM}g_inventor_not_disambiguated.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_unzip(zip_instead(applicant_g_p), os.path.dirname(applicant_g_p))
    else:
        print(f"{Fore.GREEN}[init]: {Style.DIM}g_inventor_not_disambiguated.tsv{Style.NORMAL} already exists.{Style.RESET_ALL}")

    if not os.path.exists(location_g_p):
        print(f"\n{Fore.YELLOW}[init]: Downloading {Style.DIM}g_location_not_disambiguated.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        ...
        # unzip
        print(f"{Fore.LIGHTMAGENTA_EX}[init]: Unzipping {Style.DIM}g_location_not_disambiguated.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_unzip(zip_instead(location_g_p), os.path.dirname(location_g_p))
    else:
        print(f"{Fore.GREEN}[init]: {Style.DIM}g_location_not_disambiguated.tsv{Style.NORMAL} already exists.{Style.RESET_ALL}")

    if not os.path.exists(assignee_g_p):
        print(f"\n{Fore.YELLOW}[init]: Downloading {Style.DIM}g_assignee_not_disambiguated.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        ...
        # unzip
        print(f"{Fore.LIGHTMAGENTA_EX}[init]: Unzipping {Style.DIM}g_assignee_not_disambiguated.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_unzip(zip_instead(assignee_g_p), os.path.dirname(assignee_g_p))
    else:
        print(f"{Fore.GREEN}[init]: {Style.DIM}g_assignee_not_disambiguated.tsv{Style.NORMAL} already exists.{Style.RESET_ALL}")

    if not os.path.exists(wipo_g_p):
        print(f"\n{Fore.YELLOW}[init]: Downloading {Style.DIM}g_wipo_technology.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_download("https://s3.amazonaws.com/data.patentsview.org/download/g_wipo_technology.tsv.zip", zip_instead(wipo_g_p))

        # unzip
        print(f"{Fore.LIGHTMAGENTA_EX}[init]: Unzipping {Style.DIM}g_wipo_technology.tsv{Style.NORMAL}...{Style.RESET_ALL}")
        os_specific_unzip(zip_instead(wipo_g_p), os.path.dirname(wipo_g_p))
    else:
        print(f"{Fore.GREEN}[init]: {Style.DIM}g_wipo_technology.tsv{Style.NORMAL} already exists.{Style.RESET_ALL}")

    print(f"{Fore.GREEN}[init]: {Style.NORMAL}All raw tables downloaded.{Style.RESET_ALL}")
